...
export default function ViewRequests(props) {
  ...
  async function verifySignature(buyerAddress, signature) {
    ...
    let messageHash = web3.utils.sha3(address);
    ...
    if (verificationOutput) {
      ...
      if (role === "1") {
        ...
        const itemProducto = new web3.eth.Contract(ItemProducto.abi, address);
        itemProducto.methods.updateWholesalerAddress(buyerAddress).send({ from: account });
        alert('Response sent to wholesaler');
      } else if (role === "4") {
        const itemProducto = new web3.eth.Contract(ItemProducto.abi, address);
        itemProducto.methods.updateDistributorAddress(buyerAddress).send({ from: account });
