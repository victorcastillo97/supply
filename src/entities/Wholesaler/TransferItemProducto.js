...
export default function TransferItemProducto(props) {
    const [account] = useState(props.account);
    const [web3, setWeb3] = useState(props.web3);
    const [supplyChain] = useState(props.supplyChain);
    const [loading, isLoading] = useState(false);
    const [itemProductoAddress, setItemProductoAddress] = useState("");
    ...
    const handleInputChange = (e) => {
        if (e.target.id === 'itemProductoAddress') {
            setItemProductoAddress(e.target.value);
    ...
    const handleSubmit = (e) => {
        e.preventDefault();
        isLoading(true);
        supplyChain.methods.transferItemProductoWtoD(itemProductoAddress, transporterAddress, distributorAddress).send({ from: account })
                ...
                <div className={classes.paper}>
                    ...
                    <form className={classes.form} noValidate>
                        <Grid container spacing={2}>
                            ...
                            <Grid item xs={12}>
                                <TextField variant="outlined" onChange={handleInputChange} required fullWidth id="itemProductoAddress" label="Package Address" name="itemProductoAddress" />
                            </Grid>
                            <Grid item xs={12}>
