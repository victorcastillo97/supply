...
export default function ItemProductoInfo(props) {
  ...
  function getSupplyChainStepContent(stepIndex) {
    switch (stepIndex) {
      case 0:
        return 'ItemProducto is at manufacturing stage in the supply chain.';
      case 1:
        return 'ItemProducto collected by the Transporter is on its way to the Wholesaler.';
      case 2:
        return 'ItemProducto currently with the Wholesaler';
      case 3:
  ...
  function sendPackage() {
    ...
    supplyChain.methods.sendPackageToEntity(wholesaler, account, itemProductoAddress, signature).send({ from: account })
  ...
  if (loading) {
    ...
    return (
      <div>
        ...
        <p>{details}</p>
        <Button variant="contained" color="primary" ><Link to={{ pathname: `/manufacturer/view-request/${itemProductoAddress}`, query: { address: itemProductoAddress, account: account, web3: web3, supplyChain: supplyChain } }}>View Requests</Link></Button>&nbsp;&nbsp;&nbsp;
